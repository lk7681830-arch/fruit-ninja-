...
-- FruitNinja(AU)[32+64]-v15.01--APEX[GG]v2
-- Written on v 3.73.0
LUA = 'FruitNinja(AU)[32+64]-v15.01'
UPDATE = 'Script Version : 21 Nov 2024'
GLabel = 'Fruit Ninja'
GProcess = 'com.halfbrick.fruitninjafree'
...
OFF="× " ON="√ "
HB=OFF IM=OFF SL=OFF SC=OFF 
WD=OFF BC=OFF FB=OFF NA=OFF 
xsc="" 
...
mc=gg.multiChoice({
	...
	if mc[1] then bombs() end
	if mc[2] then drop() end
	if mc[3] then location() end 
	if mc[4] then count() end
	if mc[5] then wave() end 
	if mc[6] then frenzy() end
	if mc[7] then buy() end 
	if mc[8] then noads() end 
	if mc[9] then links() end 
...
xhaX={
	...
	SL.."Spawn Location",
	SC.."Spawn Count"..xsc,
	WD.."No Delay Next Wave", 
	FB.."Frenzy Time/Stack",
	BC.."Buy Stars + Gems  [x64]", 
	NA.."No Ads PopUp", 
	}
xhaX=table.concat(xhaX, "\n")
xhaX=tostring(xhaX) 

	if mc[10] then exit() return end 
...
-- public class ModifierBomb : IModifier //
A[1].method="ShouldBeDeflected" 

function bombs()
if A[1].error==1 then error() return end 
x=1 HACK(1,1,HB) HB=ONOFF(HB) 
end 

--███████████████████████

-- public class LivesGameOverManager : IGameOverManager //
A[2].method="UpdateLivesBalance" 

function drop()
if A[2].error==1 then error() return end 
x=0 HACK(2,2,IM) IM=ONOFF(IM) 
end 
...
A[3].class="Spawner"
A[3].method="GetNextFruitPosition" 
A[4].class="Spawner"
A[4].method="GetNextFruitVelocity"
A[5].class="Spawner"
A[5].method="GetNextFruitAcceleration"

function location()
ERR(3,5) if E==1 then error() return end 
x=0 HACK(3,5,SL) SL=ONOFF(SL) 
end 
...
-- public class Spawner : MonoBehaviour //
A[6].class="Spawner"
A[6].method="GetNumberOfFruitToSpawn" 

function count()
if A[6].error==1 then error() return end 
fcnt=gg.multiChoice({"OFF / Default","x10","x15","x20  ( Lag + )","x25  ( Lag++ )","x50  ( Lag+++ )"},{},"SELECT SPAWN COUNT\n*Higher Numbers Can Lag Game")
    if fcnt==nil then cancel() return end 
    SC=OFF 
    if fcnt[1] then SC=ON x1=1776 xsc="" end 
    if fcnt[2] then x1=10 xsc=" = x10" end
    if fcnt[3] then x1=15 xsc=" = x15" end
    if fcnt[4] then x1=20 xsc=" = x20" end
    if fcnt[5] then x1=25 xsc=" = x25" end
    if fcnt[6] then x1=50 xsc=" = x50" end
    
    if is64 then 
        x1=string.format("%X",tostring(x1)) 
        x="~A8 MOV X0, #0x"..x1
    else
        x="~A MOVW R0, #"..x1
    end

HACK(6,6,SC) SC=ONOFF(SC) 
end 
	

--███████████████████████

-- public class ModifierArcadeFrenzy : IModifier //
A[7].class="ModifierArcadeFrenzy"
A[7].method="EndBananaEffect" 

function frenzy()
if A[7].error==1 then error() return end 
x=0 HACK(7,7,FB) FB=ONOFF(FB) 
end 
...
function buy()
    if not is64 then 
        gg.alert("×× BUY STARS / GEMS ××\nThis Hack is for x64 Only","OK",nil, xTAGx) 
        return
    end 
    
    if xbuy==0 then
        clear() wait() x="CurrencyConversion" t=4 o=0x20 class()
        x=60 refine() o=16 offset() x=8000 refine() 
        check() if E==0 then error() return end 
        if gg.getResultsCount()~=1 then error() return end 
        x=10000000 edit()
        o=-8 t=32 offset() XG=gg.getResults(1) xg=gg.getResults(1)
        o=-8 t=4 offset() x=0 edit()
        o=-8 t=32 offset() XC=gg.getResults(1) 
        clear() xbuy=1
    end
    
buyx=gg.multiChoice({
	"Get 10,000,000 Stars",
	"Get 10,000000 Gems"},{},"SELECT BUY OPTION")
	if buyx==nil then cancel() return end 
	if buyx[1] then xg[1].value=XG[1].value end
	if buyx[2] then xg[1].value=XC[1].value end
gg.setValues(xg) 
gg.alert("Now Buy The 8000 Stars for 60 Gems.\nWill Recieve 10,000,000 Stars/Gems and Cost 0/Free","OK",nil,xTAGx) 
BC=ON 
end 

--███████████████████████

-- public class NoAdsPopupSystem : MonoBehaviour //
A[8].class="NoAdsPopupSystem" 
A[8].method="ShowPopup"

A[9].method="CanShowInterstitial"  -- all class 

function noads()
ERR(8,9) if E==1 then error() return end 
x=0 HACK(8,9,NA) NA=ONOFF(NA) 
end 

--███████████████████████

-- public class WaveManager : MonoBehaviour //
A[10].method="get_WaveManagerDTScale" 

function wave()
if A[10].error==1 then error() return end 
    if is64 then x={'12887BE0h','72A997C0h','1E270000h','D65F03C0h'}  -- 100m float 
        else x={'E30B0C20h','E3440CBEh','EE070A90h','EEB00A67h','E12FFF1Eh'}
    end
HACK(10,10,WD) WD=ONOFF(WD) 
end 
